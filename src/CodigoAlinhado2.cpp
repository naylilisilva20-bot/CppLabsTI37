...
bool pesquisa_por_nome(const vector<string>& nomes, string nome, int& pos) {
    //
    // Localiza a pessoa pelo nome obtendo a sua posicao em todos
    // os vectores.
    bool encontrado = false;
    for (pos = 0; pos < nomes.size(); pos++) {
        if (nome == nomes[pos]) {
            encontrado = true;
            break;
        }
...
void opcao_relacao_idades(const vector<string>& nomes, const vector<int>& idades) {
    string nome;
    int pos1, pos2;

    do {
        cout << "\nIntroduza o nome da primeira pessoa: ";
        getline(cin, nome);
    } while (!pesquisa_por_nome(nomes, nome, pos1));
    
    do {
        cout << "Introduza o nome da segunda pessoa: ";
        getline(cin, nome);
    } while (!pesquisa_por_nome(nomes, nome, pos2));

    cout << endl;

    if (idades[pos1] > idades[pos2]) {
        cout << nomes[pos1] << ", " << idades[pos1]
             << ", mais velho(a) do que "<< nomes[pos2]
             << ", " << idades[pos2] << endl;
    }
    else if (idades[pos2] > idades[pos1]) {
        cout << nomes[pos2] << ", " << idades[pos2]
             << ", mais velho(a) do que "<< nomes[pos1]
             << ", " << idades[pos1] << endl;
    }
    else {
        cout << nomes[pos1]  << " e " << nomes[pos2] << " teem a mesma idade, "
             << idades[pos1] << endl;}
        cout << endl;
    }
...
void inicializacao (vector<string>& nomes, vector<int>& idades) {
